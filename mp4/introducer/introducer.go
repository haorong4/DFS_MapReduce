...
func Juice(text string) {
	...
	for i := 0; i < N; i++ {
		...
		conn.Write([]byte(message + "\n"))
		conn.Close()
		fmt.Println("Juice master: " + message)
	}
}
...
func fileList(filepath string) ([]string, error) {
	/*file_list := []string{}
	...
	for fname, _ := range fileMap {
		if strings.Contains(fname, filepath) {
...
func UpdateLocalRecord() {
	f, err := os.OpenFile("./memberlist.txt", os.O_CREATE|os.O_TRUNC|os.O_RDWR, 0777)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	f, err := os.OpenFile("./memberlist.txt", os.O_RDONLY, 0777)
	if err != nil {
		log.Fatal(err)
	}
